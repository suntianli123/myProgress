...
export async function handleUserSync(userList: any): Promise<any> {
  /* 判断锁状态 */
  // const lockStatus = await sdkInstance.middleware.etcd.getLock('async-user', 10)
  ...
  for (const user of userList) {
    ...
    if (disableList && disableList.length !== 0) {
      ...
      /* 启用用户 */
      // eslint-disable-next-line eqeqeq
      if (user.status == '0' && user.disable == '1') await handleActiveUpdate(user)
    } else {
      if (activeList.length === 0) {
        /* 新增用户 */
        await handleUserInsert(user)
      // eslint-disable-next-line eqeqeq
      } else if (user.status == '0' && user.disable == '1') {
        /* 表中存在状态为正常 */
        user.company_uid = activeList[0].company_uid
        await handleUserUpdate(user, activeList[0])
      // eslint-disable-next-line eqeqeq
      } else if (user.status == '1' || user.disable == '0') {
        /* 禁用前先更新用户信息 */
        /* 表中存在状态为正常 */
        user.company_uid = activeList[0].company_uid
        await handleUserUpdate(user, activeList[0])
  ...
  //     ? SQLResult.data.rows
  //     : []
  // const orderList = SQLUserDeptList.map((sqlItem: any) => {
  //   const obj = {
  //     company_uid: sqlItem.company_uid,
  //     dept_id: sqlItem.dept_id,
  //     order: sqlItem.user_order
  //   }
  //   return obj
  // })
  // /** 获取企业token */
  ...
  //   depts: orderList
  // })
  // const users = await getCompanyUsers(companyToken, 0, 100, 'active,notactive,disabled')
  /* 释放锁状态 */
  // await sdkInstance.middleware.etcd.releaseLock(
  //   'async-user',
  //   lockStatus.data
  // )
  // if (lockStatus.result !== 'ok') {
...
//   /* 状态为2需要删除用户 */
//   logger.info({ msg: `进入删除方法前:${JSON.stringify(user)}` })
...
async function handleUserUpdate(user: any, SQLUser: any) {
  ...
  if (user.name !== SQLUser.name && user.name !== '') {
    ...
    /* 如果错误，抛出错误 */
    if (putUsers.result !== 0) throw resErrJson({ msg: '修改企业成员失败。' })
    /* 更新用户表 */
  ...
  for (const userDept of user.insertUserDept) {
    ...
    if (userDept.dept_id !== '' && user.company_uid !== '') {
      ...
      if (insertUserDeptInfo.result !== 0) {
        throw resErrJson({ msg: '在该部门下添加成员失败。' })
      }
      /* 新增用户、部门关联表 */
...
async function handleUserDeptDelete(user: any) {
  ...
  for (const userDept of user.deleteUserDept) {
    ...
    if (deleteUserDeptInfo.result !== 0) {
      throw resErrJson({ msg: '该部门下移出成员失败。' })
    }
    /* 更新用户、部门关联表 */
    ...
    if (delMiddleUserDept.result !== 'ok') {
      throw resErrJson({ msg: 'dept_id同步用户部门关联表失败。' })
    }
  }
}
...
async function handleUserInsert(user: any) {
  ...
  if (createCompanyUsers.result !== 0) {
    logger.info({ msg: `创建成员失败:${user.id}` })
  }
  /* 激活成员 - 注释激活接口 */
  ...
  if (insertMiddleUser.result !== 'ok') {
    throw resErrJson({ msg: '新增同步用户表失败。' })
  }
  /* 如果用户状态为禁用的话需要禁用操作 */
  // eslint-disable-next-line eqeqeq
  const isNormal = user.status == '0' && user.disable == '1'
  // eslint-disable-next-line eqeqeq
  if (!isNormal) {
  ...
  for (const userDept of user.dept) {
    ...
    /* 如果错误，抛出错误 */
    if (pushUserDept.result !== 0) throw resErrJson({ msg: '同步部门失败。' })
    /* 将信息同步到关联表中 */
    const insertMiddleUserDept = await sdkInstance.middleware.mysql.insert(
      config.dbName,
      'INSERT INTO middle_user_dept (user_id, company_uid, ori_dept_id, dept_id, user_order, create_time, update_time, create_user, update_user, is_delete) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    ...
    if (pushUserDept.result !== 0) logger.info({ msg: `用户在部门下进行排序:${user.id}` })
    // eslint-disable-next-line eqeqeq
    if (user.status == '1' || user.disable == '0') {
      /* 禁用用户 */
      user.company_uid = createCompanyUsers.company_uid
      await handleUserDisable(user) // 禁用
    }
...
async function handleActiveUpdate(user: any) {
  ...
  if (enableMiddleUser.result !== 'ok') {
    throw resErrJson({ msg: '启用更新用户表失败。' })
  }
}
/* 禁用用户 */
