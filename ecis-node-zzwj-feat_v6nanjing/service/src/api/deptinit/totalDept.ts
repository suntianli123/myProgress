...
export async function totalDeptSync(deptList: any, deptListOrigin: any[]): Promise<any> {
  ...
  // 修改部门二级标识
  await updataSecondId(deptListOrigin)
  /* 释放锁状态 */
...
async function handleRecursionDeptSync(deptList: any, deptListOrigin: any[]): Promise<any> {
  ...
  if (
    deptList.length === 1 &&
    `${deptList[0].parentId}` === `${config.department.parentId}` &&
    !SQLDeptList.length
  ) {
    /* 获取wps根部门 */
...
async function handleCheckNeedHandle(
  ...
  for (const sqlDept of curSQLDeptList) {
    ...
    if (!isNeedDel && isNeedMove) {
      ...
      deleteList.push(sqlDept)
    }
    /* 没有查询到就是需要删除表里的数据 */
    // if (!isNeedDel && updateList.length === 0) deleteList.push(sqlDept)
  }
...
async function handleDeptInsert(insertList: any): Promise<any> {
  ...
  for (const dept of insertList) {
    let order: number = 0
    if (dept.dept_order && dept.dept_order !== '0') {
      // 权重
      order = 10000 - dept.dept_order
    }
    /* WPS创建部门 */
    ...
    logger.info({
      msg: `创建部门,dept: ${JSON.stringify(dept)}`
    })
    ...
    let secDeptData: any = ''
    let thirdDeptData: any = ''
    if (dept?.org_dept_second_id) {
      const secList = dept?.org_dept_second_id.split(';_')
      if (secList.length > 3) {
        const secdeptId = secList[2].split(':_')
        secDeptData = secdeptId[1]
      }
      if (secList.length > 3) {
        const thirdDeptId = secList[3].split(':_')
        thirdDeptData = thirdDeptId[1]
      }
    }
    /* 存中间表 */
    /* 存入中间表的是接口返回的数据 */
    const insertData = await sdkInstance.middleware.mysql.insert(
      config.dbName,
      'INSERT INTO middle_dept (ori_dept_id, ori_dept_name, ori_dept_pid, dept_id, dept_id_pid, dept_order, create_time, update_time, create_user, update_user, is_delete, org_dept_second_id, org_dept_third_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [
        dept.departmentId,
        dept.department,
        dept.parentId,
 ...
 * @return {result} 部门id
 */
export async function updataSecondId(deptListOrigin?: any[]) {
  for (const dept of deptListOrigin) {
    /* 获取表里部门数据 */
    const curDeptSQLResult: any = await sdkInstance.middleware.mysql.select(
      config.dbName,
      'SELECT * FROM middle_dept WHERE is_delete=0 and ori_dept_id IN(?)',
      [dept.departmentId]
    )
    const curSQLDeptList =
    curDeptSQLResult.data &&
    ...
    if (!curSQLDeptList || curSQLDeptList.length == 0) {
      logger.info(`部门在表中不存在${JSON.stringify(dept)}`)
      continue
    }
    let secDeptData: any = ''
    let thirdDeptData: any = ''
    ...
    // eslint-disable-next-line eqeqeq
    if (secDeptData && secDeptData != curSQLDeptList[0].org_dept_second_id) {
      const time = await currentTime()
      const secUpdataSql = await sdkInstance.middleware.mysql.update(
        config.dbName,
        'UPDATE middle_dept SET update_time=?, org_dept_second_id=? WHERE is_delete=0 AND ori_dept_id=?',
        [time, secDeptData, dept.departmentId]
      )
      logger.info({
        msg: `修改部门-中间表:${dept.departmentId}`
      })
      /* 如果错误，抛出错误 */
      if (secUpdataSql.result !== 'ok') {
        logger.warn(`修改二级部门标识中间表失败,dept: ${JSON.stringify(dept)},secUpdataSql: ${JSON.stringify(secUpdataSql)}`)
      }
    }
    // eslint-disable-next-line eqeqeq
    if (thirdDeptData && thirdDeptData != curSQLDeptList[0].org_dept_third_id) {
      const time = await currentTime()
      const thirdUpdataSql = await sdkInstance.middleware.mysql.update(
        config.dbName,
        'UPDATE middle_dept SET update_time=?, org_dept_third_id=? WHERE is_delete=0 AND ori_dept_id=?',
        [time, thirdDeptData, dept.departmentId]
      )
      logger.info({
        msg: `修改部门-中间表:${dept.departmentId}`
      })
      /* 如果错误，抛出错误 */
      if (thirdUpdataSql.result !== 'ok') {
        logger.warn(`修改三级部门标识中间表失败,dept: ${JSON.stringify(dept)},secUpdataSql: ${JSON.stringify(thirdUpdataSql)}`)
      }
    }
  }
}
