...
import { getAllDeptFromDB, getAllUserDeptFromDB } from '.'
...
export async function totalUserSync(userList: any): Promise<any> {
  ...
  logger.info({ msg: `查询中间表后状态为正常的:${JSON.stringify(activeList)}` })
  logger.info({
...
async function handleCheckNeedDept(userList: any, SQLUserList: any) {
  ...
  const sqlUserDeptList = await getAllUserDeptFromDB()
  for (const user of userList) {
    logger.info({ msg: `三方用户和部门信息:${JSON.stringify(user)}` })
    for (let i = 0; i < user.dept.length; i++) {
      /* 查询中间表所有用户 */
      const SQLUserDepts = sqlDeptList.filter(
        dept => dept.ori_dept_id === user.dept[i].departmentId
      )
      if (!SQLUserDepts || SQLUserDepts.length === 0) {
        logger.warn({ msg: `中间表部门不存在！请新同步部门后重试。 ${JSON.stringify(user.dept[i])}` })
        continue
      }
    ...
    for (const sqlUser of SQLUserList) {
      ...
      if (user.id === sqlUser.user_id) {
        ...
        /* 查该用户在表中所有部门 */
        const SQLUserDeptList = sqlUserDeptList.filter(userDept => userDept.user_id === user.id)
        let insertList = []
        const deleteList = []
...
async function handleDelUser(delUserList: any) {
  for (const delUser of delUserList) {
    await handleUserDisable(delUser)
