...
export async function readExcel(ctx: Context) {
  let res
  try {
    // const fileResource = await fileWriteStream(ctx)
    const fileResource = path.join(__dirname, '../../public/temporary/whiteuser.xlsx')
    // 解析excel
    const excelData = loadDataFromExcel(fileResource, true)
    // 映射字段
    const mapData = mapDataExcelResult(excelData, [
    ...
    logger.info(`数据,${JSON.stringify(mapData)}`)
    const { 白名单用户: thirdWhite } = mapData
    logger.info(`白名单数据,${JSON.stringify(thirdWhite)}`)
    if (!thirdWhite || thirdWhite.length === 0) {
      logger.info('白名单用户数据为空')
      return
    }
    for (const user of thirdWhite) {
      ...
      if (SQLUserList.length === 0) {
        ...
        logger.info(`此白名单用户已存在,${JSON.stringify(user)}`)
      }
    }
    res = resJson()
  } catch (e) {
...
async function handleUserInsert(user: any) {
  ...
  logger.info({ msg: `白名单创建成员-中间表:${user.userid}` })
  /* 返回结果判断 */
  if (insertMiddleUser.result !== 'ok') {
    logger.warn({ msg: `新增白名单用户表失败,user: ${JSON.stringify(user)},insertMiddleUser: ${JSON.stringify(insertMiddleUser)}` })
  }
}
