...
export async function handleDeptEdit(dept: any) {
  ...
  if (!curSQLDeptList || curSQLDeptList.length === 0) {
    ...
    if (
      ...
      logger.info(`该部门不存在父级部门,dept:${JSON.stringify(dept)}`)
    } else {
      /* 表里没有查询到需要新增 */
...
async function handleDeptMove(dept: any): Promise<any> {
  ...
  if (!moveSQLDept || moveSQLDept.length === 0) {
    logger.info({ msg: `移动部门-当前部门无父部门-不可移动: ${JSON.stringify(dept)}` })
  } else {
    ...
    if (editDept.result !== 0) {
      logger.warn(`wps部门移动失败,dept:${JSON.stringify(dept)},editDept:${JSON.stringify(editDept)}`)
      return
    }
    /* 更新中间表 */
    const moveData = await sdkInstance.middleware.mysql.update(
      config.dbName,
      'UPDATE middle_dept SET ori_dept_name=?,ori_dept_pid=?, dept_id_pid=?, dept_order=?, update_time=? WHERE is_delete=0 AND dept_id=?',
      [dept.department, dept.parentId, moveSQLDept[0].dept_id, order, time, dept.dept_id]
    )
    logger.info({
