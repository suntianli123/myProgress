...
export async function totalUserSync(userList: any): Promise<any> {
  ...
  logger.info({ SQLUserCount: `用户数量,SQLUserCount.data.rows：${JSON.stringify(SQLUserCount?.data?.rows)}` })
  logger.info({ SQLUserCount: `用户数量,SQLUserCount.data.rows：${JSON.stringify(SQLUserCount?.data?.rows[0])}` })
  ...
  if (disableList && disableList.length !== 0) {
    ...
    for (const SQLUser of disableList) {
      ...
      const isNeedChange = userList.some(
        ...
        (user: any) => user.id === SQLUser.user_id && (user.status == '2000')
      )
...
async function handleAddUser(insertUserList: any): Promise<any> {
  ...
  for (const user of insertUserList) {
    ...
    for (const userDept of user.dept) {
      ...
      logger.info({ msg: `修改成员order排序，WPS接口:${createCompanyUsers.company_uid}` })
      /* 如果错误，抛出错误 */
...
async function handleUpdateUser(updateUserList: any) {
  ...
  for (const user of updateUserList) {
    ...
    for (const userDept of user.dept) {
      ...
      /* 返回结果判断 */
      if (delMiddleUser.result !== 'ok') {
        logger.info({ msg: `更新用户表失败,user: ${JSON.stringify(user)},delMiddleUser: ${JSON.stringify(delMiddleUser)}` })
      }
    }
  }
}
...
async function handleActiveUpdate(user: any) {
  ...
  if (SQLUserDeptLists.length !== 0) {
    ...
    const delMiddleUser = await sdkInstance.middleware.mysql.update(
      config.dbName,
      'UPDATE middle_user_dept SET user_order=?, syncSequence=?, maxSyncSequence=? WHERE is_delete=0 AND company_uid=?',
      [user.order, user.syncSequence, user.maxSyncSequence, user.company_uid]
    )
    logger.info({ msg: `更新用户表-中间表:${user.company_uid}` })
    /* 返回结果判断 */
    if (delMiddleUser.result !== 'ok') {
