...
import { resJson } from '../../util/msgCode'
...
export async function mastUserSync(userList: any): Promise<any> {
  /* 同步用户 */
  for (const user of userList) {
    ...
    if (disableList && disableList.length !== 0) {
      ...
      /* 启用用户 */
      // eslint-disable-next-line eqeqeq
      if (user.status == '1') await handleActiveUpdate(user)
    } else {
      // eslint-disable-next-line eqeqeq
      ...
      // eslint-disable-next-line eqeqeq
        ...
        /* 表中存在状态为正常 */
        if (activeList.length && activeList[0].company_uid) {
...
async function handleUserUpdate(user: any, SQLUser: any) {
  ...
  if (user.name !== SQLUser.name && user.name !== '') {
    ...
    const delMiddleUser = await sdkInstance.middleware.mysql.update(
      ...
      [user.name, time, user.syncSequence, user.maxSyncSequence, user.id]
    )
  ...
  for (const userDept of user.insertUserDept) {
    ...
    if (userDept.dept_id !== '' && user.company_uid !== '') {
      ...
      if (insertUserDeptInfo.result !== 0) {
        logger.warn({ msg: `在该部门下添加成员失, user: ${JSON.stringify(user)}, userDept: ${JSON.stringify(userDept)}, insertUserDeptInfo: ${JSON.stringify(insertUserDeptInfo)}` })
        continue
      }
      /* 新增用户、部门关联表 */
...
async function handleUserDeptDelete(user: any) {
  ...
  for (const userDept of user.deleteUserDept) {
    ...
    if (deleteUserDeptInfo.result !== 0) {
      logger.warn({ msg: `wps侧删除成员失败,user: ${JSON.stringify(user)}, userDept: ${JSON.stringify(userDept)}, deleteUserDeptInfo: ${JSON.stringify(deleteUserDeptInfo)}` })
      continue
    }
    /* 更新用户、部门关联表 */
    ...
    if (delMiddleUserDept.result !== 'ok') {
      logger.warn({ msg: `同步用户部门关联表失败,user: ${JSON.stringify(user)}, userDept: ${JSON.stringify(userDept)}, delMiddleUserDept: ${JSON.stringify(delMiddleUserDept)}` })
    }
  }
}
...
async function handleUserInsert(user: any) {
  ...
  for (const userDept of user.dept) {
    ...
    if (pushUserDept.result !== 0) {
      logger.warn({ msg: `将企业成员同步到部门失败,user: ${JSON.stringify(user)},pushUserDept: ${JSON.stringify(pushUserDept)}` })
      continue
    }
    /* 将信息同步到关联表中 */
    const insertMiddleUserDept = await sdkInstance.middleware.mysql.insert(
    ...
    if (pushUserDept.result !== 0) logger.warn({ msg: `用户在部门下进行排序:${JSON.stringify(user)}` })
  }
}
...
async function handleUserDelete(user: any) {
  ...
  for (const userDept of user.dept) {
    ...
    if (delUserDept.result !== 0) {
      logger.warn({ msg: `从部门里移出失败,user: ${JSON.stringify(user)},userDept: ${JSON.stringify(userDept)},delUserDept: ${JSON.stringify(delUserDept)}` })
      continue
    }
    /* 删除中间表 */
    const delMiddleUserDept = await sdkInstance.middleware.mysql.delete(
  ...
  /* 如果错误，抛出错误 */
  if (delUser.result !== 0) logger.warn({ msg: `删除用户失败,user: ${JSON.stringify(user)},delUser: ${JSON.stringify(delUser)}` })
  /* 更新用户表 */
  const delMiddleUser = await sdkInstance.middleware.mysql.update(
    config.dbName,
    'UPDATE middle_users SET is_delete=1, syncSequence=?, maxSyncSequence=? WHERE is_delete=0 AND company_uid=?',
    [user.syncSequence, user.maxSyncSequence, user.company_uid]
  )
  logger.info({ msg: `更新用户表-中间表:${user.company_uid}` })
  /* 返回结果判断 */
  if (delMiddleUser.result !== 'ok') {
...
async function handleUserDisable(user: any) {
  ...
  /* 返回结果判断 */
  if (delMiddleUser.result !== 'ok') {
    logger.warn({ msg: `禁用更新用户表失败,user: ${JSON.stringify(user)},delMiddleUser: ${JSON.stringify(delMiddleUser)}` })
  }
}
