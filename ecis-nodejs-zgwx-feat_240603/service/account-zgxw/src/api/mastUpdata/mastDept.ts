...
import { resCheck, resJson } from '../../util/msgCode'
...
export async function mastDeptSync(deptList: any): Promise<any> {
  logger.info({
    msg: '进入同步方法'
  })
  /* 同步部门 */
...
export async function handleDeptEdit(dept: any) {
  ...
  if (!curSQLDeptList || curSQLDeptList.length === 0) {
    ...
    dept.dept_id = curSQLDeptList[0].dept_id
    // 部门名称和排序修改时更新
    if (dept.department !== curSQLDeptList[0].ori_dept_name || (dept.order && Number(1000000000 - dept.order) !== curSQLDeptList[0].dept_order)) {
