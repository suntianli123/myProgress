...
export async function handleDeptEdit(dept: any) {
  ...
  if (!curSQLDeptList || curSQLDeptList.length === 0) {
    ...
    dept.dept_id = curSQLDeptList[0].dept_id
    // 部门名称和排序修改时更新
    if (dept.department !== curSQLDeptList[0].ori_dept_name || (dept.order && Number(dept.order) !== curSQLDeptList[0].dept_order)) {
...
async function handleDeptMove(dept: any): Promise<any> {
  ...
  if (!moveSQLDept || moveSQLDept.length === 0) {
    ...
    if (editDept.result !== 0) {
      logger.warn(`移动部门失败:${JSON.stringify(editDept)},三方部门信息dept: ${JSON.stringify(dept)}`)
    }
    /* 更新中间表 */
    ...
    if (delData.result !== 'ok') {
      logger.warn(`更新部门中间表失败delData:${JSON.stringify(delData)},三方部门信息dept: ${JSON.stringify(dept)}`)
    }
  } else {
    const editDept = await putDepts(companyToken, dept.dept_id, {
...
async function handleDeptInsert(dept: any): Promise<any> {
  ...
  if (pushDepts.result !== 0) {
    logger.warn(`创建部门失败,pushDepts: ${JSON.stringify(pushDepts)},dept: ${JSON.stringify(dept)}`)
    return
  }
  /* 存中间表 */
  /* 存入中间表的是接口返回的数据 */
  ...
  if (insertData.result !== 'ok') {
    logger.warn(`插入部门中间表失败insertData:${JSON.stringify(insertData)},三方部门信息dept: ${JSON.stringify(dept)}`)
  }
}
...
async function handleDeptUpdate(dept: any): Promise<any> {
  ...
  /* 如果错误，抛出错误 */
  if (delData.result !== 'ok') {
    logger.warn(`修改部门-中间表失败,dept: ${JSON.stringify(dept)},delData: ${JSON.stringify(delData)}`)
  }
}
...
export async function handleDeptDelete(dept: any): Promise<any> {
  ...
  for (const deptUser of deptUsers) {
    ...
    if (delMiddleUserDept.result !== 'ok') {
      logger.warn(`删除用户部门关联表失败delMiddleUserDept:${JSON.stringify(delMiddleUserDept)},三方部门信息dept: ${JSON.stringify(dept)}`)
      continue
    }
  }
  /* 调用删除部门接口 */
  ...
  if (delDeptInfo.result !== 0) {
    logger.warn({ msg: `wps侧删除部门失败,dept: ${JSON.stringify(dept)}delDeptInfo: ${JSON.stringify(delDeptInfo)}` })
    return
  }
  ...
  if (delDeptData.result !== 'ok') {
    logger.warn(`删除部门中间表失败delDeptData:${JSON.stringify(delDeptData)},三方部门信息dept: ${JSON.stringify(dept)}`)
  }
}
